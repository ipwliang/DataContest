...
![VisDrone 数据集](./figure/VisDrone.png "VisDrone 数据集")
...
- 精度（Precision）：
精度是指在所有被模型预测为正类别的样本中，实际真正为正类别的样本所占的比例。其计算公式为：$Precision = \frac{TP}{TP + FP}$，其中 $TP$（True Positive）表示真正例，即模型正确预测为正类别的样本数；$FP$（False Positive）表示假正例，即模型错误预测为正类别的实际负类别样本数。在无人机交通监测场景下，精度反映了模型对交通目标（如车辆、行人等）检测的准确性，高精度意味着模型在检测到交通目标时，误报的概率较低。例如，当模型检测到 100 个车辆目标，其中有 90 个是真正的车辆，那么精度为 90%。这一指标对于确保交通监测系统的可靠性至关重要，因为它直接关系到系统是否会频繁发出虚假警报，干扰交通管理人员的判断。
- 召回率（Recall）：
召回率是在所有实际为正类别的样本中，模型正确预测为正类别的样本所占的比例。其计算公式为：$Recall = \frac{TP}{TP + FN}$，其中 $FN$（False Negative）表示假负例，即模型错误预测为负类别的实际正类别样本数。在交通监测任务中，召回率衡量了模型对实际交通目标的覆盖能力，高召回率意味着模型能够有效地发现大部分交通目标，避免遗漏重要的交通信息。例如，在实际存在 100 个车辆的交通场景中，模型成功检测出 80 个，则召回率为 80%。这一指标对于保证交通监测系统的全面性非常关键，它确保系统能够尽可能多地捕捉到实际的交通目标，避免因目标遗漏而导致的交通管理漏洞。
- F1 得分（F1 Score）：
F1 得分是精度和召回率的调和平均数，其计算公式为：$F1 = 2 \times \frac{Precision \times Recall}{Precision + Recall}$。它综合考虑了精度和召回率两个指标，提供了一个平衡的性能度量。在无人机交通监测任务中，F1 得分有助于评估模型在精度和召回率之间的平衡能力。例如，当模型的精度为 90%，召回率为 80% 时，其 F1 得分为 84.6%。这一指标对于评估模型在实际应用中的整体性能非常有用，因为在一个实际的交通监测系统中，既需要高精度以避免误报，又需要高召回率以确保不遗漏重要目标，F1 得分能够全面反映模型在这两方面的综合表现。
- 平均精度均值（mAP）：
作为衡量目标检测模型准确性的关键指标，适用于评估模型在识别跨境交通中多种目标类别（车辆、人员等）时的综合性能。它计算模型在不同类别目标上的查准率（Precision）和召回率（Recall）曲线下的面积平均值，取值范围在 0-1 之间，值越高表明模型在不同置信度阈值下对目标的检测越精准，越能全面地发现和正确分类交通场景中的各类目标，尤其在处理目标尺度变化大、遮挡严重等复杂情况时，能有效反映模型的鲁棒性和泛化能力。例如，在一个包含多个交通目标类别的测试集上，模型对每个类别的目标检测结果绘制出精度 - 召回率曲线，计算每条曲线下的面积，然后求取这些面积的平均值作为 mAP。这一指标能够全面衡量模型在复杂交通场景下的整体检测性能，是评估交通监测系统优劣的重要依据。
- 推理时间（Inference Time）：
直接体现模型在实际应用中的实时性表现，以毫秒（ms）为单位，衡量从输入一帧无人机视频图像到输出检测结果所需的时间。对于跨境交通实时监测系统而言，较短的推理时间至关重要，它确保系统能够及时处理快速变化的交通场景，为交通管理部门提供即时有效的决策依据。一般要求推理时间控制在[X]ms 以内，以满足无人机视频流的高帧率实时处理需求，避免出现监测数据滞后导致交通拥堵等问题无法及时响应的情况。例如，如果无人机采集的视频帧率为 15fps，则推理时间需要控制在约 66ms 以内才能保证实时处理，否则会出现数据积压和延迟，影响系统的实时监测效果。
- FPS：
